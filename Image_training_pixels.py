...
background"""
    ...
    for i in range(0, n): #begins the counter on number of images based on 
			  #the input
        img = Image.new('L', [250, 250]) #creates a 250x250 pixel image
        draw = ImageDraw.Draw(img)
        np.random.seed(123+i) #creates reproducibility in randomness that 
			      #changes with each iteration
        for j in range(0,img.size[0]): #Creates random starting points for 
				       #pixels
...
def pixel_counter_whole_array(array):
    ...
    pixel_count = pd.DataFrame(columns=['Index','Object', 
                                        'Black', 'Total'])#Creates dataframe 
for storing count data
