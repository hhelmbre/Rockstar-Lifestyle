...
# 	Team members (alphabetical): Julia Boese, 
#                                    Hawley Helmbrecht, 
#                                    Sage Scheiwiller, 
#                                    David Shackelford
#
...
#       manipulation
#
# Note: Current version only supports png type images for stack creating; 
#       will add futher functionality for tif and tif ij images as well
#
...
class imgID():
    # Creating an identifier for every image for use in neural network
...
def stackMRH(stacked_img):
    ...
    for k in range(len(stacked_img[:,:,0])):
        plt = []
        obj = imgID(dict({'Image' : '{}/{}'.format((k+1),len(
                    stacked_img[:,:,0]))}), stacked_img[k,:,:])
...
def stackLoad(path = '', file = ''):
	"""
	The following function loads the given cropped stacked image 
        into a numpy arrays
	Parameters
	----------
...
def imgCrop(path = '', file = '',
	...
	"""
	The following function acts as a wrapper for the entire py file. 
        It will input an image file and given resolution and output a 
        stacked set of cropped tif images. It also saves the set
	into a tif image set on the current folder

	* Current notes: I only have this running for a png input for 
        the purposes of our project, it will only output the green 
        band of the images
	...
	def __imgSave(td, stacked_img):
		"""
		The following private function saves the given image 
                            array to a tif file with given metadata
		...
		Return
		------
		stacked_img : np.uint16 : (n, res_x, res_y) array of n 
                                    stacks of cropped images
		"""
		...
		tifffile.imsave('{}stack_crop_RES{}_{}{}'.format(
                                td.path, td.res, td.file, '.tif'), 
                                stacked_img, metadata = metadata, 
                                imagej = ij)

	def __crop(td):
		"""
		The following private function crops an image attached 
                to object td based on its applied resolution
		...
		Return
		------
		img_stack : np.uint16 : (n, res_x, res_y) array of n 
                stacks of cropped images
		"""
		...
		img_stack = np.zeros(shape = (0, 0, 0, 0), 
                                     dtype = 'uint16')
		...
		for j in range(len(_img[0,:])//res_x):
		    for i in range(len(_img[:,0])//res_y):
		        ...
		        # 	     j*offset[1]+border[1],j*offset[1]
                        #            +border[1]+offset[1])
